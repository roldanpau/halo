...
#include "int_rtbp.h"			// mu, DIM, int_rtbp_exit
...
/**
 * \brief Integrate IC to section and find optimal correction.
 *
 * Given an initial condition that is very close to the nominal halo periodic
 * orbit, integrate it up to Poincare section {y=0}. Then find a (tiny)
 * correction that ensures orbit stays on the LPO region for as long as
 * possible, effectively shadowing the halo.
 *
 * \remark For convenience, the state after applying the maneuver, \f$ q90 +
 * \Delta v\f$, is returned in q90_new.
 *
 * \remark If the maneuver is not able to guarantee that orbit remains inside
 * the LPO region for at least SHADOW_TIME, the procedure is considered
 ...
 * @param[in]		q_Masde		Initial condition (pos-vel coordinates)
 *
 * @param[in]		sec		Poincare section SEC1 {y=0, vy>0} or SEC2 {y=0,
 * vy<0}
 *
 * @param[in]		SHADOW_TIME		Extra time (after integrating to section)
 * we must ensure we stay shadowing the halo.
 *
 * @param[in]      corr   		Type of correction maneuver
 * @param[out]		q90         State after integrating to section (pos-vel)
 * @param[out]		q90_new     q90 after maneuver (pos-vel coordinates)
 * @param[out]		ti			integration time to section
 *
 * @returns	dv	Modulus of correction maneuver (or 0, if procedure fails).
 */
		...
		double *ti)
{
	...
	/* Integrate orbit to Poincare section */
	prtbp(mu, sec, 1, q, ti);
